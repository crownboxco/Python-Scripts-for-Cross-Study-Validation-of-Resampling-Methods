# This script is so we could run the HMF1 model selection on a separate machine.
import numpy as np
import pandas as pd
...
OUT_TUNING_CSV = "PLSDA_BestVal_SelectedLV_HighMacroF1_Dataset.csv"
...
def choose_best_lv(inner_scores):
    """Pick LV with highest mean inner Macro F1; ties favor smaller LV."""
    stats = {}
    for lv, vals in inner_scores.items():
        ...
        stats[int(lv)] = (mean, sd, sd / np.sqrt(len(vals)) if len(vals) > 1 else 0.0, len(vals))

    if not stats:
        return np.nan, np.nan, np.nan, np.nan, 0

    lv = min(stats, key=lambda k: (-stats[k][0], k))
    mean, sd, se_val, n = stats[lv]
    return lv, mean, sd, se_val, n
...
print("\n" + "=" * 80)
print("PLS-DA resampling comparison: highest mean inner Macro F1 LV selection")
print("=" * 80)
...
for outer_i, (tr_idx_all, te_idx_all) in enumerate(sss.split(np.zeros(n_units), y_units_all), start=1):
    ...
    for k in KFOLDS:
        if can_do_stratified_kfold(y_train_units, k):
            ...
            splits = list(skf.split(np.zeros(len(train_units)), y_train_units))
            methods.append((f"KFCV (K={k})", splits))
        else:
    ...
    for strategy, splits in methods:
        ...
        chosen_lv, val_mean, val_sd, val_se, k_inner = choose_best_lv(inner_scores)

        rows_out.append(
            {
                "OuterSplit": outer_i,
                "Strategy": strategy,
                "ChosenLV_BestVal": chosen_lv,
                "ChosenValF1_MeanAcrossInner": val_mean,
                "ChosenValF1_SD": val_sd,
                "ChosenValF1_SE": val_se,
                "ChosenValF1_k_inner": k_inner,
                "ChosenTestF1": float(test_f1_by_lv.get(chosen_lv, np.nan)) if np.isfinite(chosen_lv) else np.nan,
...
# -------------------- Summary --------------------
chosen_df = pd.DataFrame(rows_out)
...
summary = (
    ...
    .agg(
        ...
        ChosenLV_Median=("ChosenLV_BestVal", "median"),
        ChosenLV_SE=("ChosenLV_BestVal", se),
        ChosenValF1_Median=("ChosenValF1_MeanAcrossInner", "median"),
        ChosenValF1_SE=("ChosenValF1_MeanAcrossInner", se),
        ChosenTestF1_Median=("ChosenTestF1", "median"),
        ChosenTestF1_SE=("ChosenTestF1", se),
...
for strategy in summary["Strategy"]:
    ...
    test_stats.append(wilcoxon_lr(sub["ChosenTestF1"], sub["Baseline_TestF1"]))
    lv_stats.append(wilcoxon_lr(sub["ChosenLV_BestVal"], sub["BaselineLV"]))

summary["wilcoxon_TestF1_n_pairs_used"] = [x[2] for x in test_stats]
summary["median_diff_TestF1_(chosen-baseline)"] = [x[3] for x in test_stats]
summary["wilcoxon_TestF1_p_right_(H1_chosen>baseline)"] = [x[0] for x in test_stats]
summary["wilcoxon_TestF1_p_left_(H1_chosen<baseline)"] = [x[1] for x in test_stats]

summary["wilcoxon_LV_n_pairs_used"] = [x[2] for x in lv_stats]
summary["median_diff_LV_(chosen-baseline)"] = [x[3] for x in lv_stats]
summary["wilcoxon_LV_p_right_(H1_chosenLV>baselineLV)"] = [x[0] for x in lv_stats]
summary["wilcoxon_LV_p_left_(H1_chosenLV<baselineLV)"] = [x[1] for x in lv_stats]

if OUT_TUNING_CSV:
    summary.to_csv(OUT_TUNING_CSV, index=False)

print("\n" + "=" * 80)
print("DONE")
print("=" * 80)
if OUT_TUNING_CSV:
    print(f"Saved: {OUT_TUNING_CSV}")
print("\nChosen-LV summary:")
